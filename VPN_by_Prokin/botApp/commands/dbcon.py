...
def get_user_balance(message):
    return  execute_query(f"SELECT balance from users where telegram_id = '{message.from_user.id}'")[0]
...
def create_support_task(message):
    ...
    list_tasks = execute_query(f"SELECT id FROM support_tasks WHERE telegram_id = '{telegram_id}' and state = 1", fetch_one=False)
    tasks = str()
    for i in list_tasks:
        tasks = tasks + str(i[0]) + ", "
    return tasks
...
def get_user_traffic(message):
    return execute_query(f"select traffic from users_vpn_keys where user_name = '{message.from_user.id}'",fetch_one=False)[0]
