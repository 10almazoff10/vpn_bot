...
from vpn_bot.commands import dbcon, outline_api_reqests
...
PRICE_PER_MOUNTH = config.PRICE_PER_MOUNTH
...
def send_give_price():
    users_balance = dbcon.execute_query(
        """select telegram_id, balance, user_state, id FROM users where balance <= 5 and user_state != 1;""", False)
        ...
        if float(user[1]) > -5:
            ...
            try:
                bot.send_message(user[0],
                                 f"""Уважаемый пользователь, Ваш баланс менее 5 рублей, пожалуйста, нажмите 'Пополнить' чтобы пополнить счет\nНапоминаю что при балансе менее -5 рублей, доступ будет заблокирован""")
